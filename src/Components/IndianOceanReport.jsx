// src/components/IndianOceanReport.jsx

import React, { useState } from 'react';
import { FaDownload, FaShareAlt, FaExpand } from 'react-icons/fa';
import IndianOceanGeographicalContent from './IndianOceanGeographicalContent';

const IndianOceanReport = () => {
    const [activeTab, setActiveTab] = useState('statistical');

    const handleDownload = () => {
        const link = document.createElement('a');
        if (activeTab === 'statistical') {
            link.href = '/Indian_Ocean_Statistical.html';
            link.download = 'Indian_Ocean_Statistical_Report.html';
        } else {
    ...
    const handleShare = () => {
        if (navigator.share) {
            navigator.share({
                title: 'Indian Ocean Report',
                text: `Check out this data profiling report for the Indian Ocean!`,
                ...
                <div className="max-w-5xl mx-auto">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                        <div>
                            ...
                            <h1 className="text-3xl md:text-4xl font-bold text-text-light dark:text-text-dark">Indian Ocean Profiling Report</h1>
                        </div>
                    ...
                    <div className="w-full h-[80vh] bg-background-light dark:bg-subtle-dark border border-subtle-light dark:border-subtle-dark rounded-xl shadow-lg overflow-hidden">
                        {activeTab === 'statistical' ? (
                            <iframe className="w-full h-full border-0" src="/Indian_Ocean_Statistical.html" title="Indian Ocean Statistical Report"></iframe>
                        ) : (
                            <IndianOceanGeographicalContent />
                        )}
                    </div>
...
export default IndianOceanReport;